...
export default class PackagePanelView {
  ...
  addPackage (pack, timeKey) {
    ...
    const upperThreshold = atom.config.get('timecop-2.upperThreshold')

    if (pack[timeKey] < 5) {
      timeCode.classList.add('inline-block', 'highlight-success')
    } else {
      timeCode.classList.add('inline-block', pack[timeKey] > upperThreshold ? 'highlight-error' : 'highlight-warning')
    }
