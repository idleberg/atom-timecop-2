...
import { pluralize } from 'underscore-plus'
...
export default class Timecop2View {
  constructor ({ uri }) {
    this.uri = uri
  ...
  showLoadedPackages () {
    ...
    this.refs.packageLoadingPanel.refs.summary.textContent = dedent`
      ...
      ${pluralize(packages.length, 'package')} took longer than ${atom.config.get('timecop-2.lowerThreshold')}ms to load.
    `
  }
  ...
  showActivePackages () {
    ...
    this.refs.packageActivationPanel.refs.summary.textContent = dedent`
      ...
      ${pluralize(packages.length, 'package')} took longer than ${atom.config.get('timecop-2.lowerThreshold')}ms to activate.\
    `
  }
  ...
  showLoadedThemes () {
    ...
    this.refs.themeLoadingPanel.refs.summary.textContent = dedent`
      ...
      ${pluralize(packages.length, 'theme')} took longer than ${atom.config.get('timecop-2.lowerThreshold')}ms to load.\
    `
  }
  ...
  showActiveThemes () {
    ...
    this.refs.themeActivationPanel.refs.summary.textContent = dedent`
      ...
      ${pluralize(packages.length, 'theme')} took longer than ${atom.config.get('timecop-2.lowerThreshold')}ms to activate.\
    `
  }
  ...
  getSlowPackages (packages, timeKey) {
    ...
    packages = packages.filter(function (pack) {
      time += pack[timeKey]
      count++
      return pack[timeKey] > atom.config.get('timecop-2.lowerThreshold')
    })
  ...
  getLowerThreshold () {
    return atom.config.get('timecop-2.lowerThreshold')
  }
}
